...
static void stdout_callback(log_Event *ev) { //向控制台写
  ...
  // #elif __linux
  fprintf((FILE*)ev->udata, "%s \x1b[1m%s%-5s %s:%d:\x1b[0m ", buf,
...
static void file_callback(log_Event *ev) { //向文件写
  ...
  fprintf((FILE*)ev->udata, "%s %-5s %s:%d: ", buf, LOG_LEVEL_STRING[ev->level],
          ev->file, ev->line);
  vfprintf((FILE*)ev->udata, ev->fmt, ev->ap);
  fprintf((FILE*)ev->udata, "\n");
  fflush((FILE*)ev->udata);
...
void filelog_close() {
  if (FILE_LOGGING.udata) {
    fclose((FILE*)FILE_LOGGING.udata);
  }
}

void log_log(int level, const char *file, int line, const char *fmt, ...) {
  printf("aaaaa");fflush(stdout);
  log_Event ev = {
      .fmt = fmt,
      .file = file,
      .line = line,
